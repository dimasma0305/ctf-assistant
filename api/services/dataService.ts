...
export async function calculateYearlyRanks(year: number): Promise<Map<string, number>> {
    ...
    // Try to get from cache first
    let yearlyRanks = cache.getCached<Map<string, number>>(cacheKey);
    ...
    if (!yearlyRanks) {
        try {
            ...
            // Calculate user scores for this specific year
            const yearlyUserScores = await FairScoringSystem.calculateUserScores(yearQuery);

            // Convert to ranked list
            const sortedUsers = Array.from(yearlyUserScores.values())
                .sort((a, b) => b.totalScore - a.totalScore);

            // Create rank map
            yearlyRanks = new Map<string, number>();
            sortedUsers.forEach((user, index) => {
                yearlyRanks!.set(user.userId, index + 1);
            });

            // Cache for 24 hours (yearly data doesn't change often)
            cache.set(cacheKey, yearlyRanks, 24 * 60 * 60 * 1000);

        } catch (error) {
            console.error(`Error calculating yearly ranks for ${year}:`, error);
            return new Map();
        }
    }
