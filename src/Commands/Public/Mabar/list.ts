...
export const command: SubCommand = {
  data: new SlashCommandSubcommandBuilder()
    .setName("list")
    .setDescription("List all registered mabar notification channels in this server"),
    ...
    try {
      ...
      if (registrations.length === 0) {
        await interaction.editReply({
          content: "📭 No channels are registered for mabar notifications in this server.\n\n" +
                   "Use `/mabar register` to register a channel!"
        });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        .setTitle("📢 Registered Mabar Notification Channels")
        .setColor(0x00AE86)
        .setDescription(`Found ${registrations.length} registered mabar channel(s) in this server`)
        .setTimestamp();
