...
export const command: SubCommand = {
  data: new SlashCommandSubcommandBuilder()
    .setName("register")
    .setDescription("Register the current channel to receive CTF mabar notifications")
    ...
    try {
      ...
      if (existingRegistration) {
        if (existingRegistration.is_active) {
          ...
          await interaction.editReply({
            content: `✅ Channel ${targetChannel} has been re-activated for CTF mabar notifications!\n\n` +
                     `📢 Subscribed to:\n${eventTypesList}`
          });
        }
        return;
      ...
      await interaction.editReply({
        content: `✅ Successfully registered ${targetChannel} to receive CTF mabar notifications!\n\n` +
